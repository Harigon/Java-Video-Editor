...
public class Timeline extends JPanel {
	...
	public void setMousePosition(int x, int y) {
		mouseX = x;
		mouseY = y;
		MainApplet.getInstance().getTimeline2().requestFocusInWindow();
		...
		if(keyId == 127){//Remove
			if(selectedTrackObject != null){
				if(selectedTrackObject instanceof TrackItem){
					TrackManager.removeItem((TrackItem) selectedTrackObject);
				}
			}
		}
		else if(keyId == KeyEvent.VK_S){//Split
			if(selectedTrackObject != null){
				if(selectedTrackObject instanceof TrackItem){
					
					TrackItem trackItem = (TrackItem) selectedTrackObject;
					
					int position = TimelineManager.timelinePosition - trackItem.getTrackStartPosition();
					
					TrackManager.splitItem(trackItem, position);
				}
			}
		} 
		else if(keyId == KeyEvent.VK_SPACE){//play/pause
			if(PlayEvent.playing == false){
				MainApplet.play();
			} else if(PlayEvent.playing){
				MainApplet.stopPlaying();
			}
		}
		else {
			System.out.println("Unhandled key press: "+keyId);
		}
	}
	...
	public void mousePressed(int x, int y) {
		...
		if (draggedTrackItem != null) {
			...
			Track track = src.screens.editorScreen.timeline.track.TrackManager
					.getTrackForItem(draggedTrackItem);
			...
			itemDraggedOffset = x
					- src.screens.editorScreen.timeline.track.TrackManager
							.trackPositionToPixelPosition(draggedTrackItem
	...
	protected void paintComponent(Graphics g) {
		...
		try {
			...
			// img =
			// Toolkit.getDefaultToolkit().getImage("C:/Users/Harry/Pictures/h.png");
			super.paintComponent(g);

			Dimension size = this.getSize();

			int timelineLength = src.screens.editorScreen.timeline.track.TrackManager
					.trackPositionToPixelPosition(Renderer.pixels.length);
			...
			if (MainApplet.getInstance() != null) {
				if (MainApplet.getInstance().getjScrollPane2() != null) {
					if (MainApplet.getInstance().getjScrollPane2()
							.getVerticalScrollBar() != null) {
						startX = 20 - MainApplet.getInstance()
								.getjScrollPane2().getVerticalScrollBar()
								.getValue();
					}
				}
			...
			if (src.screens.editorScreen.timeline.TimelineManager.tracks != null) {
				...
				/*
				 * Store a local version of the global track array-list (To
				 * prevent any concurrency issues).
				 */
				ArrayList<Track> tempTracks = (ArrayList<Track>) src.screens.editorScreen.timeline.TimelineManager.tracks
						.clone();
				...
				for (Track trackInstance : tempTracks) {
					...
					g.drawRect(x + 1, y + 1, (((int) size.getWidth()) - 1) - 2,
							trackHeight - 2);

					if (mouseX > x && mouseY > y
							&& mouseX < x + size.getWidth()
							&& mouseY < y + trackHeight) {

						int hoveredX = src.screens.editorScreen.timeline.track.TrackManager
								.pixelPositionToTrackPosition(mouseX);
						if (src.screens.editorScreen.timeline.track.TrackManager
								.getItemAtTrackRegion(trackInstance, mouseX,
										mouseX + 30) == null) {

							if ((trackInstance instanceof VideoTrack && (MediaPanel.draggedMediaItem instanceof MediaImageItem || MediaPanel.draggedMediaItem instanceof MediaVideoItem))
									|| (trackInstance instanceof AudioTrack && MediaPanel.draggedMediaItem instanceof MediaAudioItem)) {
					...
					/*
					 * Check if a track item is currently being dragged.
					 */
					if (draggedTrackItem != null) {
						if (!trackInstance.trackItems
								.contains(draggedTrackItem)) {
							if (mouseY >= y && mouseY <= y + trackHeight) {
								src.screens.editorScreen.timeline.track.TrackManager
										.moveItemToNewTrack(draggedTrackItem,
												trackInstance);
							}
						}
					}
					...
					for (TrackItem trackItem : trackInstance.trackItems) {
						...
						int itemX = (src.screens.editorScreen.timeline.track.TrackManager
								.trackPositionToPixelPosition(trackItem.trackStartPosition));
						int itemLength = (src.screens.editorScreen.timeline.track.TrackManager
								.trackPositionToPixelPosition(trackItem.mediaDuration
										- (trackItem.trackStartPosition - trackItem
												.getTrackStartPosition())) - 2);
						int itemY = y + 4;
						...
						for (Transition transition : trackInstance.transitions) {
							...
							if (transition.item1 == trackItem) {
								// int startPoint =
								// trackPositionToPixelPosition((transition.item1.trackStartPosition+transition.item1.mediaDuration)-transition.duration);

								int duration = src.screens.editorScreen.timeline.track.TrackManager
										.trackPositionToPixelPosition(transition.duration);
								int start2 = (itemX + itemLength) - duration;

								if (mouseX >= start2 && mouseY >= itemY
										&& mouseX <= start2 + duration
								...
								if (transition == selectedTrackObject) {
									...
									g.drawRect(start2, itemY, duration,
											itemHeight);
								} else {
								...
								g.fillRect((start2) + 1, itemY + 1,
										duration - 1, itemHeight - 1);

								g.setColor(new Color(58, 110, 165));
								g.fillRect((start2) + 3, itemY + 3,
										duration - 5, itemHeight - 5);
						...
						if (mouseX >= itemX && mouseY >= itemY
								&& mouseX <= itemX + itemLength
								&& mouseY <= itemY + itemHeight) {
							hoveredTrackItem = trackItem;

							TrackItem possibleTransitionItem = src.screens.editorScreen.timeline.track.TrackManager
									.getItemAtTrackRegion(trackInstance, itemX
											+ itemLength + 1,
											(itemX + itemLength) + 1);

							// boolean unused =
							// TransitionManager.itemPartOfAnyTransition(trackItem)
							// == null &&
							// TransitionManager.itemPartOfAnyTransition(possibleTransitionItem)
							// == null;
							...
							if (TransitionMediaPanel.hoveredTransition != TransitionMediaPanel.draggedTransition
									&& possibleTransitionItem != null
									&& mouseX > (itemX + itemLength) - 10
									&& unused) {
						...
						if (src.screens.editorScreen.timeline.track.TrackManager
								.getItemAtTrackRegion(trackInstance, itemX - 5,
										itemX) == null) {
						...
						if (mouseX >= (itemX + itemLength) - 5
								&& mouseY >= itemY && mouseX <= rightTrimEndX
								&& mouseY <= itemY + itemHeight) {
							itemTrimHover = 2;
							hoveredTrackItem = trackItem;
						}

						if (mouseX >= leftTrimStartX && mouseY >= itemY
								&& mouseX <= itemX + 5
								&& mouseY <= itemY + itemHeight) {
							itemTrimHover = 1;
							hoveredTrackItem = trackItem;
						...
						if (trackItem == draggedTrackItem) {
							...
							int currentPosition = trackItem
									.getTrackStartPosition();

							for (TrackItem trackItem2 : trackInstance.trackItems) {
								int endingPosition = trackItem2
										.getTrackStartPosition()
										+ trackItem2.mediaDuration;
							...
							if (!src.screens.editorScreen.timeline.track.TrackManager
									.validTrackItemPosition(draggedTrackItem)) {
								g.setColor(new Color(147, 0, 0));
								g.fillRect(itemX + 1, itemY + 1,
										itemLength - 1, itemHeight - 1);
							} else {
								g.setColor(new Color(201, 201, 217));
								g.fillRect(itemX + 1, itemY + 1,
										itemLength - 1, itemHeight - 1);
							...
							g.setColor(new Color(201, 201, 217));
							g.fillRect(itemX + 1, itemY + 1, itemLength - 1,
									itemHeight - 1);
...
.getInstance(AlphaComposite.SRC_OVER, 0.7f));
						...
						int barHeight = 5;
						int barY = itemY+itemHeight-barHeight-3;
						
						int barX = itemX+3;
						int barLength = itemLength-6;
						
						g.setColor(new Color(0, 0, 0));
						g.drawRect(barX,barY,barLength,barHeight);
						
						g.setColor(new Color(12, 73, 97));
						g.fillRect(barX+1, barY+1, barLength-1, barHeight-1);
						
						g.setColor(new Color(112, 146, 190));
						g.fillRect(barX+1, barY+1, barLength-60, barHeight-1);*/
				...
				g.drawRect(
						...
						10,
						(src.screens.editorScreen.timeline.track.TrackManager
								.trackPositionToPixelPosition(Renderer.pixels.length)) - 1,
						7);
				for (int index = 0; index < Renderer.renderingStatus.length; index++) {
					...
					g.fillRect(
							src.screens.editorScreen.timeline.track.TrackManager
									.trackPositionToPixelPosition(index),
							10 + 1, 1, 7 - 1);
				}
				...
				g.fillRect(
						src.screens.editorScreen.timeline.track.TrackManager
								.trackPositionToPixelPosition(src.screens.editorScreen.timeline.TimelineManager.timelinePosition),
						0, 1, 400);
	...
	public void drawTrackItemThumbnails(Graphics g, Track trackInstance,
			TrackItem trackItem, int itemX, int itemY, int itemLength) {

		if ((trackInstance instanceof VideoTrack && (trackItem instanceof TrackImageItem || trackItem instanceof TrackVideoItem))
				|| trackInstance == null) {
			...
			if (itemLength >= spaceNeeded2) {
				g.drawImage(image, ((itemX + itemLength) - width) - 2,
						itemY + 3, null);
			} else {
				int newWidth = itemLength - 6;
				try {
					g.drawImage(cropImage(image, newWidth, image.getHeight()),
							((itemX + itemLength) - newWidth) - 2, itemY + 3,
							null);
				} catch (Exception e) {
