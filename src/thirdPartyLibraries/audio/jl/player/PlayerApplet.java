...
public class PlayerApplet extends Applet implements Runnable
{
	 ...
	 */
	protected AudioDevice getAudioDevice(int decreasedVolume) throws JavaLayerException
	{
		return FactoryRegistry.systemRegistry().createAudioDevice(decreasedVolume);
	}
	 ...
	 */
	protected void play(InputStream in, AudioDevice dev, int decreasedVolume) throws JavaLayerException
	{
		stopPlayer();
		
		if (in!=null && dev!=null)
		{
			player = new Player(in, dev, decreasedVolume);
	 ...
	 */
	public void start(int decreasedVolume)
	{
		String name = getAudioFileName();
		try
		{			
			InputStream in = getAudioStream();
			AudioDevice dev = getAudioDevice(decreasedVolume);
			play(in, dev, decreasedVolume);
		}
