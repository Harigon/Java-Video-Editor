...
public class AVI_Reader  {
   ...
   //the input file
    ...
    public void run(File file){
        try {
            openAndReadHeader(file.getAbsolutePath());                                //open and read header
        } catch (Exception e) {
        	e.printStackTrace();
        }                        //ask for parameters
        try {
        	...
        	isVirtual = true;
        	verbose = true;
            makeStack(file.getAbsolutePath(), firstFrame, lastFrame, isVirtual, convertToGray, flipVertical);  //read data
        } catch (Exception e) {
    ...
    public BufferedImage getFrame(int index){
		...
		try {
			...
			pixels = (int[]) readFrame(rFile, frameInfo[0], (int)frameInfo[1]);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		
		BufferedImage bi = getBI(pixels, dwWidth, dwHeight);
     ...
     */
    public void makeStack (String path, int firstFrame, int lastFrame,
    ...
    public int getSize() {
        if (getFrameInfos() == null) return 0;
        else return getFrameInfos().size();
    }
    ...
    public void deleteSlice(int n) {
        if (getFrameInfos()==null || getFrameInfos().size()==0) return;
        if (n<1 || n>getFrameInfos().size())
            throw new IllegalArgumentException("Argument out of range: "+n);
        getFrameInfos().removeElementAt(n-1);
    }
    ...
    private void readAVI(String path) throws Exception, IOException {
        ...
        if (isVirtual || firstFrame>1) {        // avoid scanning frame-by-frame where we only need the positions
            setFrameInfos(new Vector<long[]>(100)); // holds frame positions, sizes and time since start
            long nextPosition = -1;
            ...
            if (hasIndex && getFrameInfos().size()==0) {  // got nothing from indx, attempt to read AVI 1 index 'idx1'
            ...
            if (verbose)
                System.out.println("'frameInfos' has "+getFrameInfos().size()+" entries");
        }
        if (isVirtual && getFrameInfos().size()>0)   // Virtual Stack only needs reading the index
            return;
        // Read AVI (movie data) frame by frame - if no index tag is present the pointers
        // for the virtual AVI stack will be read here
        raFile.seek(headerPositionEnd);
        if (firstFrame>1 && getFrameInfos().size()>0) {
            long[] frameInfo = (long[])getFrameInfos().get(0);
            raFile.seek(frameInfo[0]-8);        // chunk starts 8 bytes before frame data
            frameNumber = firstFrame;
            if (verbose)
                System.out.println("directly go to frame "+firstFrame+" @ 0x"+Long.toHexString(frameInfo[0]-8));
        ...
        //System.out.println("at 0x"+Long.toHexString(pos)+" filesize=0x"+Long.toHexString(fileSize));
    ...
    private void openAndReadHeader (String path) throws Exception, IOException {
        ...
        if (verbose)
            System.out.println("OPEN AND READ AVI FILE HEADER "+timeString());
        ...
        if (verbose)
            System.out.println("File header: File type='"+fourccString(fileType)+"' (should be 'RIFF')"+timeString());
        ...
        if (verbose)
            System.out.println("File header: RIFF type='"+fourccString(riffType)+"' (should be 'AVI ')");
    ...
    private void readAVIX(long endPosition) throws Exception, IOException {
        if (verbose)
            System.out.println("Trying to read AVIX"+timeString());
        int riffType = readInt();
        if (verbose)
            System.out.println("File header: RIFF type='"+fourccString(riffType)+"' (should be 'AVIX')");
    ...
    private long findFourccAndRead(int fourcc, boolean isList, long endPosition,
        ...
        do {
            ...
            if (isList && type == FOURCC_LIST)
                type = readInt();
            if (verbose)
                System.out.println("Searching for '"+fourccString(fourcc)+"', found "+fourccString(type)+
                        "' "+posSizeString(nextPos-size, size));
            if (type==fourcc) {
                contentOk = readContents(fourcc, nextPos);
            } else if (verbose)
                System.out.println("Discarded '"+fourccString(type)+"': Contents does not fit");
            raFile.seek(nextPos);
            if (contentOk)
    ...
    private long findFourccAndSkip(int fourcc, boolean isList, long endPosition) throws IOException {
        while (true) {
            ...
            if (isList && type == FOURCC_LIST)
                type = readInt();
            if (verbose)
                System.out.println("Searching for (to skip) '"+fourccString(fourcc)+"', found "+fourccString(type)+
        ...
        switch (fourcc) {
            ...
            case FOURCC_strh:
                ...
                if (streamType != FOURCC_vids) {
                    if (verbose)
                        System.out.println("Non-video Stream '"+fourccString(streamType)+" skipped");
                    streamNumber++;
                    return false;
                }
                readStreamHeader();
    ...
    void readAviHeader() throws Exception, IOException {        //'avih'
        ...
        if (verbose) {
            System.out.println("AVI HEADER (avih):"+timeString());
    ...
    void readStreamHeader() throws Exception, IOException {     //'strh'
        ...
        if (verbose) {
            System.out.println("VIDEO STREAM HEADER (strh):");
            System.out.println("   fccStreamHandler='" + fourccString(fccStreamHandler)+"'");
            System.out.println("   dwStreamFlags=" + dwStreamFlags);
            System.out.println("   wPriority,wLanguage=" + dwPriorityLanguage);
    ...
    private void readAvi2Index(long endPosition) throws Exception, IOException {
        ...
        if (verbose) {
            ...
            System.out.println("AVI 2 INDEX:");
            System.out.println("   wLongsPerEntry=" + wLongsPerEntry);
            System.out.println("   bIndexSubType=" + bIndexSubType);
            System.out.println("   bIndexType=" + bIndexType + bIndexString);
            System.out.println("   nEntriesInUse=" + nEntriesInUse);
            System.out.println("   dwChunkId='" + fourccString(dwChunkId)+"'");
            if (bIndexType == AVI_INDEX_OF_CHUNKS)
                System.out.println("   qwBaseOffset=" + "0x"+Long.toHexString(qwBaseOffset));
        }
        if (bIndexType == AVI_INDEX_OF_INDEXES) {       // 'indx' points to other indices
            ...
            for (int i=0;i<nEntriesInUse;i++) {
                ...
                if (verbose) {
                 //     System.out.println("qwOffset:"+qwOffset);
                    System.out.println("   index data '" +fourccString(dwChunkId)+"' "+posSizeString(qwOffset,dwSize)+timeString());
                }
            ...
            if (dwChunkId != type0xdb && dwChunkId != type0xdc) { //not the stream we search for? (should not happen)
                if (verbose)
                    System.out.println("INDEX ERROR: SKIPPED ix00, wrong stream number or type, should be "+
            ...
            for (int i=0;i<nEntriesInUse;i++) {
                ...
                if (frameNumber >= firstFrame) {
                    getFrameInfos().add(new long[] {pos, dwSize, (long) frameNumber*dwMicroSecPerFrame});
                    //if (verbose)
                    //    System.out.println("movie data "+frameNumber+" '"+fourccString(dwChunkId)+"' "+posSizeString(pos,dwSize)+timeString());
                }
                frameNumber++;
                if (frameNumber>lastFrameToRead) break;
            }
            if (verbose)
                System.out.println("Index read up to frame "+(frameNumber-1));
        }
    }
    ...
    private void readOldFrameIndex(long endPosition) throws Exception, IOException {
        ...
        while (true) {
            ...
            int dwSize = readInt();
            //System.out.println("idx1: dwOffset=0x"+Long.toHexString(dwOffset));
            //System.out.println("moviPosition=0x"+Long.toHexString(moviPosition));
            if ((dwChunkId==type0xdb || dwChunkId==type0xdc) && dwSize>0) {
                if (offset < 0) {       // find out what the offset refers to
                    ...
                    for (int i=0; i<offsetsToTry.length; i++) {
                        ...
                        //System.out.println("read@=0x"+Long.toHexString(pos)+":  '"+fourccString(chunkIdAtPos)+"'");
                    ...
                    if (verbose)
                        System.out.println("idx1: dwOffsets are w.r.t. 0x"+(offset<0 ? " UNKONWN??" : Long.toHexString(offset)));
                    raFile.seek(temp);
                    if (offset < 0) return;                 // neither offset works
                }
                if (frameNumber >= firstFrame) {
                    long framepos = (dwOffset+offset) & SIZE_MASK;
                    getFrameInfos().add(new long[]{framepos+8, dwSize, (long)frameNumber*dwMicroSecPerFrame});
                    //if (verbose)
                        //System.out.println("idx1 movie data '"+fourccString(dwChunkId)+"' "+posSizeString(framepos,dwSize)+timeString());
                }
                frameNumber++;
                if (frameNumber>lastFrameToRead) break;
            } //if(dwChunkId...)
        ...
        if (verbose)
            System.out.println("(Old) Index read up to frame "+(frameNumber-1));
    ...
    */
    void readBitMapInfo(long endPosition) throws Exception, IOException {
        ...
        if (verbose) {
            ...
            System.out.println("   biPlanes=" + biPlanes);
            System.out.println("   biBitCount=" + biBitCount);
            System.out.println("   biCompression=0x" + Integer.toHexString(biCompression)+" '"+fourccString(biCompression)+"'");
            System.out.println("   biSizeImage=" + biSizeImage);
            System.out.println("   biXPelsPerMeter=" + biXPelsPerMeter);
            System.out.println("   biYPelsPerMeter=" + biYPelsPerMeter);
            System.out.println("   biClrUsed=" + biClrUsed);
            System.out.println("   biClrImportant=" + biClrImportant);
        }
        ...
        if (verbose) {
            ...
            System.out.println("   > palette colors=" + biClrUsed);
        ...
        if (readPalette) {
            ...
            if (verbose)
                System.out.println("   Reading "+biClrUsed+" Palette colors: " + posSizeString(spaceForPalette));
    ...
    void readMovieData(long endPosition) throws Exception, IOException {
    	...
    	if (verbose)
            System.out.println("MOVIE DATA "+posSizeString(endPosition-raFile.getFilePointer())+timeString());
        if (verbose)
            System.out.println("Searching for stream "+streamNumber+": '"+
                    fourccString(type0xdb)+"' or '"+fourccString(type0xdc)+"' chunks");
        if (isVirtual) {
            if (getFrameInfos()==null)                       // we might have it already from reading the first chunk
                setFrameInfos(new Vector<long[]>(100));   // holds frame positions in file (for non-constant frame sizes, should hold long[] with pos and size)
        }
        while (true) {                                  //loop over all chunks
            ...
            if ((type==type0xdb || type==type0xdc) && size>0) {
                if (verbose)
                    System.out.println(frameNumber+" movie data '"+fourccString(type)+"' "+posSizeString(size)+timeString());
                if (frameNumber >= firstFrame) {
                    if (isVirtual)
                        getFrameInfos().add(new long[]{pos, size, frameNumber*dwMicroSecPerFrame});
                    else {                       //read the frame
                ...
                System.out.println("skipped '"+fourccString(type)+"' "+posSizeString(size));
    ...
    public Object readFrame (RandomAccessFile rFile, long filePos, int size)
            throws Exception, IOException {
        rFile.seek(filePos);
        //if (verbose)
        //    System.out.println("virtual AVI: readFrame @"+posSizeString(filePos, size));
    ...
    private Object readCompressedFrame (RandomAccessFile rFile, int size)
        ...
        if (img==null) throw new Exception("can't read frame, ImageIO returns null");
    ...
    private int readType(long endPosition) throws IOException {
        while (true) {
            ...
            long size = readInt()&SIZE_MASK;
            if (verbose)
                System.out.println("Skip JUNK: "+posSizeString(size));
    ...
    public Vector<long[]> getFrameInfos() {
		return frameInfos;
	}

	public void setFrameInfos(Vector<long[]> frameInfos) {
    ...
    class raInputStream extends InputStream {
        ...
        public int read (byte[] b, int off, int len) throws IOException {
            //System.out.println("read "+len+" bytes, fix="+fixMJPG);
            int nBytes;
            if (fixMJPG) {
        ...
        private void addHuffmanTables() {
            ...
            do {
                ...
                //System.out.println("code=0x"+Long.toHexString(code));
        ...
        private void insertHuffmanTables(int position) {
            //System.out.println("inserting Huffman tables");
